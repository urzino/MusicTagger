...
t_size = 0.91429
train_set, val_set = train_test_split(tot_train_set, train_size=t_size, test_size=(1-t_size)) 
...
while (initial_epoch <= max_epochs) and (training_nr <= max_trainings):
    ...
    print('\n\n* * * * Starting training {0} with resort {2} from epoch {1} * * * * \n\n'.format(training_nr,  initial_epoch+1, resort_nr))
    
    #update lr
    decay = global_decay ** training_nr
    learning_rate = starting_learning_rate * decay
    
    if resort_nr == max_resorts:
